...
public class AppController {
	...
	public String viewRecherche() {
		return "moteur_recherche";
	}
	
	@GetMapping("teest")
	public String viewTeestPage() {
		return "index";
	}
	...
	public String viewEmployePage(Model model) {
		...
		//System.out.println(listQcm);
		return "employe";
	}
	
	
	//page du directeur ou il verra un résumer de l'application et le détails statistiques des réponses aux questions de chaque QCM
	...
	public String recoverPass(@RequestParam("j_username") String username) {
	    //do smthin
		List <QCM> listQcmRecherche = (List<QCM>) qcmRepo.findByCategorie(username);
		return "rechercheCat";
	}
	
	//page des employé pour voir les qcm, les créer, les supprimer et demande leurs mise en ligne on y met la liste des qcm		@GetMapping("/rechercheCat")
	public String viewRechercheePage(Model model) {
		List<QCM> listQcm = qcmRepo.findAll();
		model.addAttribute("listQcmRecherche", listQcmRecherche);
		//System.out.println(listQcm);
		return "rechercheCat_resultat";
	}
