...
public interface QcmReponduRepository extends JpaRepository<QCMRepondu, Long> {
	...
	@Query(value = "SELECT count(*) FROM QCMRepondu ;", nativeQuery = true)
	public String countTotalQCMRepondu();

	@Query(value = "SELECT reponses FROM QCMRepondu where idQCM = ?1", nativeQuery = true)
	public List<String> findAllById(Long id);

	@Query(value = "SELECT reponses FROM QCMRepondu where idQCM = ?1 and id_user = ?2", nativeQuery = true)
